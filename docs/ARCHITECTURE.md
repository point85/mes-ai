...
| **Equipment** | `id`, `name`, `code`, `description`, `work_cell_id`, `equipment_class_id` (nullable FK → EquipmentClass), `state_model_id` (nullable, refs EquipmentStateModel.model_id — null = 100% available), `max_queue_depth` (nullable int — max WIP items allowed in queue, null = unlimited) | → WorkCell, → ProcessSegments (M:N), → EquipmentMaterials, → EquipmentClass, → EquipmentCapabilities |
...
| **ProcessSegment** | `id`, `route_id`, `sequence`, `name`, `step_type` (production/inspection/rework/mrb), `equipment_class_id` (nullable FK → EquipmentClass — denormalized primary class; full requirement set lives in SegmentEquipmentRequirements 1:N), `expected_cycle_time_sec`, `erp_operation_number` (nullable — ERP operation/step number for outbound reporting, e.g. '0010') | → OperationsDefinition, → EquipmentClass, → SegmentParameters, → SegmentEquipmentRequirements (1:N), → SegmentMaterialRequirements (1:N), → outgoing ProcessSegmentDependencies, → incoming ProcessSegmentDependencies |
...
| **SegmentEquipmentRequirement** | `id`, `step_id`, `equipment_class_id` (nullable FK → EquipmentClass), `equipment_id` (nullable FK → Equipment), `use_type` (required/preferred/alternate), `description` | → ProcessSegment (N:1), → EquipmentClass (N:1, optional), → Equipment (N:1, optional). Exactly one of `equipment_class_id` or `equipment_id` must be set (CHECK `ck_segment_equip_req_one_target`). Unique on (step_id, equipment_id) and on (step_id, equipment_class_id). One ProcessSegment may have many requirements — dispatch AND's them (ISA-95 Part 2 EquipmentSegmentSpecification). `ProcessSegment.equipment_class_id` is a denormalized convenience for the primary class; additional constraints live here (D050). |
