...
interface Props {
  wipType: "unit" | "lot";
  wipId: string;
  wipLabel: string; // serial_number or lot_number for display
  onClose: () => void;
}
...
export default function DispatchDialog({ wipType, wipId, wipLabel, onClose }: Props) {
  ...
  // Load strategy list and run initial evaluation (lets server resolve default)
  useEffect(() => {
    let cancelled = false;
    ...
    async function init() {
      try {
        ...
        // Evaluate with no explicit strategy → server uses work cell default
        setLoading(true);
        const res = await evaluateDispatch(
          wipType === "unit" ? { unit_id: wipId } : { lot_id: wipId },
        );
        if (cancelled) return;
        setResult(res);
        setStrategy(res.strategy); // pre-select whatever server used
        setSelectedId(res.recommended?.equipment_id ?? null);
      } catch (e: unknown) {
        if (!cancelled) setError((e as { message?: string })?.message ?? "Evaluation failed");
      } finally {
      ...
      <div className="w-full max-w-2xl rounded-xl bg-white shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              {wipType === "unit" ? "Unit" : "Lot"}: <span className="font-mono font-medium">{wipLabel}</span>
            </p>
          </div>
          <button onClick={onClose} className="rounded p-1 text-gray-400 hover:text-gray-600">
            <XMarkIcon className="h-5 w-5" />
        ...
        <div className="px-5 py-4 border-b">
          ...
          {/* Blocked / error notices */}
          {error && (
            <p className="mt-2 text-sm text-red-600">{error}</p>
          )}
          {result?.blocked && (
            <div className="mt-2 rounded-md bg-amber-50 border border-amber-200 px-3 py-2 text-sm text-amber-800">
              <span className="font-medium">Blocked:</span> {result.blocked_reason ?? "No eligible equipment found"}
            </div>
          )}
        </div>
        ...
        <div className="flex-1 overflow-auto px-5 py-4">
          {loading && !result && (
            <p className="text-sm text-gray-400 py-6 text-center">Evaluating…</p>
          )}
          {!loading && result && sortedOptions.length === 0 && !result.blocked && (
            <p className="text-sm text-gray-400 py-6 text-center">No equipment options returned.</p>
          )}
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="border-b text-left text-xs text-gray-500 uppercase tracking-wide">
                  ...
                  <th className="pb-2 pr-3">Work Cell</th>
                  <th className="pb-2 pr-3 text-right">Queue</th>
              ...
              <tbody>
                {sortedOptions.map((opt) => {
                  ...
                  return (
                    <tr
                      key={opt.equipment_id}
                      onClick={() => setSelectedId(opt.equipment_id)}
                    ...
                    >
                      ...
                      <td className="py-2 pr-3 font-mono font-medium">
                        {opt.equipment_code}
                        {opt.equipment_name && (
                          <span className="ml-1 font-sans font-normal text-gray-500">
                            {opt.equipment_name}
                          </span>
                        )}
                      </td>
                      <td className="py-2 pr-3 text-gray-600">{opt.work_cell_code}</td>
                      <td className="py-2 pr-3 text-right tabular-nums">
                        {opt.queue_depth}
                        {opt.max_queue_depth != null
                          ? ` / ${opt.max_queue_depth}`
                          : ""}
                      </td>
                      ...
                      <td className="py-2 text-gray-500 text-xs">{opt.reason ?? "—"}</td>
                    </tr>
                  );
                })}
              </tbody>
