/**
 * Area Create / Edit dialog — modal form with Zod validation.
 */
...
export default function AreaFormDialog({ area, siteId, onClose }: Props) {
  const isEdit = !!area;
  ...
  const onSubmit = async (data: AreaFormData) => {
    try {
      if (isEdit) {
        await updateMut.mutateAsync({ id: area!.id, ...data });
      } else {
        await createMut.mutateAsync({ siteId, ...data });
      }
      onClose();
    } catch {
