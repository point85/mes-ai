 ...
 *
 * When condition='disposition', the user picks from the catalog Disposition
 * dropdown (preferred) or falls back to a free-text label. Transitions with
 * disposition_id set are surfaced with full catalog metadata at runtime by
 * the routing engine's get_available_dispositions endpoint.
 */
...
const schema = z
  ...
  .refine(
    (d) =>
      d.condition !== "disposition" ||
      (d.disposition_id && d.disposition_id.trim().length > 0) ||
      (d.label && d.label.trim().length > 0),
    {
      message: "Pick a catalog disposition (or enter a free-text label)",
      path: ["disposition_id"],
    },
  );
...
export default function TransitionFormDialog({
  ...
  useEffect(() => {
    if (transition) {
      reset({
        ...
        disposition_id: transition.disposition_id ?? "",
      });
    }
  }, [transition, reset]);
  ...
  const onSubmit = async (data: FormData) => {
    try {
      const isDisp = data.condition === "disposition";
      const chosenDispId = isDisp && data.disposition_id ? data.disposition_id : null;
      // When a catalog disposition is picked, mirror its name into `label`
      // for backward-compat with older dispatcher code that reads the label.
      let label: string | null = null;
      if (isDisp) {
        if (chosenDispId) {
          const d = dispositions.find((x) => x.id === chosenDispId);
          label = d?.name ?? null;
        } else {
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <DialogPanel className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            {/* Disposition picker + label — only shown for disposition */}
              ...
              <>
                <div>
                  ...
                  <select
                    ...
                    className="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                  >
                    <option value="">— none (use custom label below) —</option>
                    {dispositions.map((d) => (
                  ...
                  {errors.disposition_id && (
                    <p className="mt-1 text-xs text-red-600">
                      {errors.disposition_id.message}
                    </p>
                  )}
                  <p className="mt-0.5 text-[11px] text-gray-400">
                    Pick a reusable catalog entry, or leave blank and use a free-text label.
                  </p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">
