...
export default function ERPCompletionPage() {
    ...
    <div className="max-w-xl space-y-4">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg border p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input type="number" value={qtyGood} onChange={(e) => setQtyGood(Number(e.target.value))} min={0} className="mt-1 w-full border rounded px-3 py-2 text-sm" required />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Qty Reject</label>
            <input type="number" value={qtyReject} onChange={(e) => setQtyReject(Number(e.target.value))} min={0} className="mt-1 w-full border rounded px-3 py-2 text-sm" />
          </div>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700">Operation Step</label>
          <select value={stepId} onChange={(e) => setStepId(e.target.value)} className="mt-1 w-full border rounded px-3 py-2 text-sm" disabled={steps.length === 0}>
            {steps.length === 0 ? (
              <option value="">&mdash; select an order first &mdash;</option>
            ) : (
              steps.map((s) => (
        ...
        <button type="submit" disabled={loading || !orderId} className="px-4 py-2 bg-green-600 text-white text-sm rounded hover:bg-green-700 disabled:opacity-50">
      ...
      {result && (
        <div className={`p-4 rounded-lg border ${result.success ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}>
          <div className="text-sm font-medium mb-1">{result.success ? "SAP Confirmation Posted" : "SAP Rejected"}</div>
