...
export default function ERPConsumptionPage() {
  ...
  const addFromBom = () => {
    ...
    setLines((prev) => [
      ...prev,
      { material_code: item.material_code, quantity: item.quantity, uom: item.uom, lot_number: "" },
    ]);
    setSelectedBomItemId("");
  };
    ...
    <div className="max-w-2xl space-y-4">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg border p-4 space-y-4">
        <div>
          ...
          <select value={orderId} onChange={(e) => setOrderId(e.target.value)} className="mt-1 w-full border rounded px-3 py-2 text-sm" required>
            <option value="">&mdash; select an order &mdash;</option>
            {orders.map((o) => (
              <option key={o.id} value={o.id}>
                {o.order_number}{o.erp_reference ? ` (${o.erp_reference})` : ""} &mdash; {o.status}
              </option>
            ))}
          </select>
        </div>
        ...
        {bomItems.length > 0 && (
          <div>
            ...
            <div className="flex gap-2">
              <select value={selectedBomItemId} onChange={(e) => setSelectedBomItemId(e.target.value)} className="flex-1 border rounded px-3 py-2 text-sm">
                <option value="">&mdash; pick a material &mdash;</option>
                {bomItems.map((item) => {
                  const step = item.route_step_id ? stepMap.get(item.route_step_id) : null;
                  return (
                    <option key={item.id} value={item.id}>
                      {item.material_code} &mdash; {item.quantity} {item.uom}
                      {step ? ` [Step ${step.sequence}: ${step.name}]` : ""}
                    </option>
                  );
                })}
              </select>
              <button type="button" onClick={addFromBom} disabled={!selectedBomItemId} className="px-3 py-2 bg-blue-600 text-white text-sm rounded hover:bg-blue-700 disabled:opacity-50">Add</button>
            </div>
          </div>
        )}
            ...
            {lines.map((line, idx) => (
              <div key={idx} className="grid grid-cols-[1fr_80px_60px_1fr_32px] gap-2 items-end">
                <div>
                  ...
                  <input type="text" value={line.material_code} readOnly className="w-full border rounded px-2 py-1.5 text-sm bg-gray-50" />
                </div>
                <div>
                  <label className="block text-xs text-gray-500">Qty</label>
                  <input type="number" value={line.quantity} onChange={(e) => updateLine(idx, "quantity", Number(e.target.value))} step="0.01" min="0.01" className="w-full border rounded px-2 py-1.5 text-sm" required />
                </div>
                <div>
                  <label className="block text-xs text-gray-500">UoM</label>
                  <input type="text" value={line.uom} readOnly className="w-full border rounded px-2 py-1.5 text-sm bg-gray-50" />
                </div>
                <div>
                  <label className="block text-xs text-gray-500">Lot #</label>
                  <input type="text" value={line.lot_number} onChange={(e) => updateLine(idx, "lot_number", e.target.value)} placeholder="optional" className="w-full border rounded px-2 py-1.5 text-sm" />
                </div>
                <button type="button" onClick={() => removeLine(idx)} className="text-red-400 hover:text-red-600 pb-1" title="Remove">&times;</button>
              </div>
            ))}
          </div>
        )}

        <button type="submit" disabled={loading || !orderId || lines.length === 0} className="px-4 py-2 bg-green-600 text-white text-sm rounded hover:bg-green-700 disabled:opacity-50">
      ...
      {result && (
        <div className={`p-4 rounded-lg border ${result.success ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}>
          <div className="text-sm font-medium mb-1">{result.success ? "Goods Movement Posted" : "SAP Rejected"}</div>
