...
def compute_working_time(
    ...
    """
    Calculate total working time (across all teams, deduplicated by minute)
    between from_dt and to_dt.
    """
    if to_dt <= from_dt:
        return timedelta(0)

    instances = compute_shift_instances_for_range(
        schedule, from_dt.date(), to_dt.date(),
    )
    total = timedelta(0)
    for inst in instances:
        overlap_start = max(inst.start_datetime, from_dt)
        overlap_end = min(inst.end_datetime, to_dt)
